...
class CTkScrollableDropdown(customtkinter.CTkToplevel):
    ...
    def __init__(self, attach, x=None, y=None, button_color=None, height: int = 200, width: int = None,
        ...
        if self.attach is not None:
            ...
            # Handle the case where self.attach is None
            print("Warning: self.attach is None.")
        #self.attach.bind('<Configure>', lambda e: self._withdraw() if not self.disable else None, add="+")
        if self.attach is not None:
        ...
        if self.attach is not None and (double_click or self.attach.winfo_name().startswith("!ctkentry") or self.attach.winfo_name().startswith("!ctkcombobox")):
            self.attach.bind('<Double-Button-1>', lambda e: self._iconify(), add="+")
        elif self.attach is None:
            print("Warning self.attach is None")
        else:
            self.attach.bind('<Button-1>', lambda e: self._iconify(), add="+")

        if self.attach is not None and self.attach.winfo_name().startswith("!ctkcombobox") :
        ...
        if self.attach is not None and self.attach.winfo_name().startswith("!ctkoptionmenu"):
    ...
    def bind_autocomplete(self, ):
        ...
        if self.attach is not None and self.attach.winfo_name().startswith("!ctkentry"):
